...
时间表示有两个标准：localtime和`UTC(Coordinated Universal Time)`。UTC是与时区无关的全球时间标准。尽管概念上有差别，UTC和GMT(格林威治时间)是一样的。localtime标准则依赖于当前时区。
...
之前的`manajaro-kde`为18版本，当时分区`/boot/efi`并没有出错，但是在这次的`manjaro-xfce-20.1`分区时安装到后期报错，
